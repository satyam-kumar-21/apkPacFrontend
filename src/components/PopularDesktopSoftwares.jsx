import React from "react";
import { Link } from "react-router-dom";

const desktopSoftwares = [
  {
    id: 1,
    name: "Mail",
    category: "Productivity",
    rating: 4.1,
    image: "https://cdn-icons-png.flaticon.com/512/561/561127.png"
  },
  {
    id: 2,
    name: "Pinterest",
    category: "Social",
    rating: 4.7,
    image: "https://cdn-icons-png.flaticon.com/512/145/145808.png"
  },
  {
    id: 3,
    name: "Zoom Workplace",
    category: "Productivity",
    rating: 3.7,
    image: "https://cdn-icons-png.flaticon.com/512/747/747314.png"
  },
  {
    id: 4,
    name: "Calculator +",
    category: "Utilities & tools",
    rating: 3.5,
    image: "https://cdn-icons-png.flaticon.com/512/992/992651.png"
  },
  {
    id: 5,
    name: "Pluto TV",
    category: "Entertainment",
    rating: 4.2,
    image: "https://cdn-icons-png.flaticon.com/512/5968/5968885.png"
  },
  {
    id: 6,
    name: "Discord",
    category: "Social",
    rating: 4.3,
    image: "https://cdn-icons-png.flaticon.com/512/5968/5968756.png"
  },
  {
    id: 7,
    name: "Bree AI - Chat Bot",
    category: "Productivity",
    rating: 4.8,
    image: "https://cdn-icons-png.flaticon.com/512/4712/4712035.png"
  },
  {
    id: 8,
    name: "Facebook",
    category: "Social",
    rating: 4.1,
    image: "https://cdn-icons-png.flaticon.com/512/733/733547.png"
  },
  {
    id: 9,
    name: "OneDrive",
    category: "Productivity",
    rating: 2.7,
    image: "https://cdn-icons-png.flaticon.com/512/732/732221.png"
  }
];

const PopularDesktopSoftwares = () => (
  <section className="w-full max-w-6xl mx-auto mt-10 mb-8 px-4">
    <h2 className="text-2xl md:text-3xl font-bold mb-6 text-gray-900">Popular Desktop Softwares</h2>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {desktopSoftwares.map((software, idx) => (
        <Link
          to={`/apps/${software.id}`}
          key={software.id}
          className="flex items-center gap-4 bg-white rounded-2xl shadow-md p-4 hover:shadow-xl transition group border border-gray-100 cursor-pointer no-underline"
          style={{ textDecoration: 'none' }}
        >
          <div className="flex-shrink-0">
            <img
              src={software.image}
              alt={software.name}
              className="w-14 h-14 rounded-xl object-contain border border-gray-200 bg-gray-100"
              loading="lazy"
            />
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <span className="font-semibold text-lg text-gray-800 truncate">
                {idx + 1}. {software.name}
              </span>
              <span className="ml-2 px-2 py-0.5 text-xs rounded bg-blue-100 text-blue-700 font-medium">
                {software.category}
              </span>
            </div>
            <div className="flex items-center gap-2 mt-1">
              <span className="text-yellow-500 font-bold text-base">★</span>
              <span className="text-gray-700 text-sm font-medium">{software.rating}</span>
            </div>
          </div>
        </Link>
      ))}
    </div>
  </section>
);

export default PopularDesktopSoftwares;
