...
const EducationAppsSection = () => {
  const { data: apps = [], isLoading } = useGetAppsQuery();
  // Filter apps for description1 category 'Education'
  const educationApps = apps.filter((app) => {
    if (app.description1) {
      const match = app.description1.match(/<td[^>]*>\s*Category\s*<\/td>\s*<td[^>]*>(.*?)<\/td>/i);
      if (match && match[1].toLowerCase() === "education") {
        return true;
      }
    }
    return false;
  });
  const showApps = educationApps.slice(0, 9);
...
export default EducationAppsSection;