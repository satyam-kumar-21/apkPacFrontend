...
const SocialAppsSection = () => {
  const { data: apps = [], isLoading } = useGetAppsQuery();
  // Filter apps for description1 category 'Social' or direct category 'social'
  const socialApps = apps.filter((app) => {
    let isSocial = false;
    if (app.description1) {
      const match = app.description1.match(/<td[^>]*>\s*Category\s*<\/td>\s*<td[^>]*>(.*?)<\/td>/i);
      if (match && match[1].toLowerCase() === "social") {
        isSocial = true;
      }
    }
    if (app.category && app.category.toLowerCase() === "social") {
      isSocial = true;
    }
    return isSocial;
  });
  const showApps = socialApps.slice(0, 9);

  return (
    <section className="w-full max-w-6xl mx-auto mt-10 mb-8 px-4">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl md:text-3xl font-bold text-gray-900">Social</h2>
        <Link
          to="/apps/social"
...
export default SocialAppsSection;