...
const ShoppingAppsSection = () => {
  ...
  // Filter apps for description1 category 'Shopping' or direct category 'shopping'
  const shoppingApps = apps.filter((app) => {
    let isShopping = false;
    if (app.description1) {
      const match = app.description1.match(/<td[^>]*>\s*Category\s*<\/td>\s*<td[^>]*>(.*?)<\/td>/i);
      if (match && match[1].toLowerCase() === "shopping") {
        isShopping = true;
      }
    }
    if (app.category && app.category.toLowerCase() === "shopping") {
      isShopping = true;
    }
    return isShopping;
  });
  const showApps = shoppingApps.slice(0, 9);
